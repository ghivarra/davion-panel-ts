...
import { imageUrl, loginUrl, checkAxiosError } from "@/libraries/Helpers"
...
const background = ref('url(' + imageUrl('login-background.jpg') + '&original=true)')
const csrfToken = ref(props.token)
const csrfHash = ref(props.hash)

// computed data
const logo = computed(() => {
    return imageUrl(`logo/${config.value.logo}`, 220)
})

// methods
const togglePasswordInput = (): void => {
...
const login = (event: Event): void => {
    ...
    axios.post(loginUrl('authenticate'), new FormData(target))  
